...
If wehave an empty folder that is where output will be placed when you run the code then we put a readme.md file in it to ensure the folder ends up in the repository.
...
No units tests are available in this project. Unit tests are great when we need to see how a macro runs and also when we need to troubleshoot code. Starting a new SAS session and reading in all the macros with a `sasautos` call will give us a clean slate to run our unit tests on to make sure our scripts are running correctly. Recently, for important pieces of code that are going to be reused a lot we have incorporated unit tests. An example can be found [here](https://github.com/nz-social-investment-agency/social_investment_data_foundation/blob/master/unittests/si_unit_test.sas).
...
* EG projects do not help with re-usability. Our code is modular and reusable by placing each macro in its own script with the file name matching the macro name and sourcing them all via a `sasautos` call. There is no need for EG projects.
* One of the SIA’s coding conventions involves appropriate naming of file scripts. We avoid spaces in file names because they can cause problems in the command line and are annoying to escape. We also avoid numbering the scripts. Instead the order of execution is dictated by a main script. That way if we need to add a new script in the middle we don’t need to renumber everything. Having a main script also means that we can do testing to ensure that code runs end to end using just one script. This makes it a lot easier for anyone new picking up the code.
