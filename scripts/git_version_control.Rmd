...
4.	Within the setup wizard, do the following:
      1.    On the information screen, click next.
      2.    On the components screen, leave the default components selected and click next.
      3.    On the path environment screen, select use Git from Bash only before clicking next.
      4.    On the backend screen, select open ssl library and click next.
      5.    On the conventions screen, select windows style commit unix style line endings and click next (this is because we may work on cross platform projects).
      6.    On the emulator screen, select MinTTY as a terminal emulator and click next.
      7.    On the extra options screen, enable caching and enable the Git credential manager before clicking install.
...
6.    Open the `gitconfig` file. It can be found either in the installation folder (C:\Program Files\Git\etc\) or within your documents folder (C:\Users\<username>). It can be editted with a text editor, such as Notepad. You should see the two commands that you typed in step 5.
...
### Cloning a repositry
Cloning is a Git command that takes a repositry from the remote repository (i.e. from GitHub) and creates a local copy on your machine. You can then make changes and subsequently push them back to the remote repository.

Within Bash, begin by changing directory to your desired working location (e.g. `cd C:/NotBackedUp`). Then enter the following command, specifying the name of the desired Git repositry (social_investment_data_foundation.git in the example code below).
...
Only the person who created the repo or the owner of the organisation can modify the settings of a repository. **Before** making a repositry public you must have sign off from the appropriate manager.
...
1. In GitHub click on the repositry name.
...
5. The hyperlink is similar to paths in the repositry. The only thing is the organisation / profile should have `.github.io` after it. An example of a GitHub page hyper link is 
...
Last updated Sep-2017 by SA on bahalf of CM
